...
@section('content')
  ...
  <div class="container">
    ...
    <div class="card">
      ...
      <form method="POST" action="{{ route('email-templates.update', $emailTemplate) }}" id="emailTemplateForm">
        @csrf
        @method('PUT')

        <div class="form-group">
          <label for="template_name">Template Name *</label>
          <input type="text" name="template_name" id="template_name" value="{{ old('template_name', $emailTemplate->template_name) }}" required>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="hidden" name="html_content" id="html_content" value="{{ old('html_content', $emailTemplate->html_content ?: $emailTemplate->body) }}">
          <input type="hidden" name="design_json" id="design_json" value="{{ old('design_json', $emailTemplate->design_json) }}">
          <input type="hidden" name="body" id="body" value="{{ old('body', old('html_content', $emailTemplate->html_content ?: $emailTemplate->body)) }}">
        </div>

        <div class="form-group">
          <label for="cc_emails">CC Emails</label>
          <input type="text" name="cc_emails" id="cc_emails" value="{{ old('cc_emails', $emailTemplate->cc_emails) }}" placeholder="email1@example.com, email2@example.com">
          <div class="help-text">Comma-separated list of email addresses to CC.</div>
        </div>

        <div class="form-group">
          <label for="bcc_emails">BCC Emails</label>
          <input type="text" name="bcc_emails" id="bcc_emails" value="{{ old('bcc_emails', $emailTemplate->bcc_emails) }}" placeholder="email1@example.com, email2@example.com">
