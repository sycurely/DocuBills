        ...
        <tbody>
          @forelse($templates as $template)
            <tr>
              ...
              <td>{{ $template->creator->username ?? 'N/A' }}</td>
              <td>
                <div style="display: flex; gap: 0.5rem;">
                  <a href="{{ route('email-templates.show', $template) }}" class="btn" style="padding: 0.5rem; background: var(--primary); color: white;">
                    <i class="fas fa-eye"></i>
                  </a>
                  @if(has_permission('edit_email_template'))
                    <a href="{{ route('email-templates.edit', $template) }}" class="btn" style="padding: 0.5rem; background: #f8961e; color: white;">
                      <i class="fas fa-edit"></i>
                    </a>
                  @endif
                  @if(has_permission('delete_email_template'))
                    <form method="POST" action="{{ route('email-templates.destroy', $template) }}" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this template?');">
                      @csrf
                      @method('DELETE')
