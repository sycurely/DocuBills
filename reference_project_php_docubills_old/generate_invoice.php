...
// Ensure selected price column is included (auto mode)
$priceColNotice = '';
if (!$manual_mode && $selected_col) {
    $priceIdx = array_search($selected_col, $data['headers'], true);
    if ($priceIdx === false) {
        $priceColNotice = 'Selected price column "' . $selected_col . '" was not found in your data. Totals may be inaccurate until you choose a valid price column.';
    } elseif (!in_array($selected_col, $includeCols, true)) {
        // Force-include price column; keep max 15 columns by dropping a non-price column if needed
        if (count($includeCols) >= 15) {
            foreach (array_reverse(array_keys($includeCols)) as $key) {
                if ($includeCols[$key] !== $selected_col) {
                    unset($includeCols[$key]);
                    break;
                }
            }
        }
        $includeCols[$priceIdx] = $data['headers'][$priceIdx];
        ksort($includeCols);
        $priceColNotice = 'Selected price column "' . $selected_col . '" was added back to keep totals accurate.';
    }
}
...
<div class="app-container">
  ...
  <div class="main-content">
    ...
    <!-- Invoice Table -->
      ...
      <?php else: ?>
        ...
        <?php if ($priceColNotice !== ''): ?>
          <div class="inline-warning"><?= htmlspecialchars($priceColNotice) ?></div>
        <?php endif; ?>
        ...
        <div id="taxSelectionWarning" class="inline-warning" style="display:none;">
          Taxable invoice is ON. Please select at least one tax checkbox (line or invoice level), or turn off Taxable invoice.
        </div>
  ...
  function isStripeBlocked() {
    ...
    return !!(warning && !warning.classList.contains('hidden') && ack && !ack.checked);
  }

  function updateSaveButtonState() {
    const saveBtn = document.getElementById('saveInvoiceBtn');
    if (!saveBtn) return;
    const taxBlocked = saveBtn.dataset.taxBlocked === '1';
    const stripeBlocked = isStripeBlocked();
    if (taxBlocked || stripeBlocked) {
  ...
  function validateTaxSelection() {
    const toggle = document.getElementById('taxableInvoiceToggle');
    const warning = document.getElementById('taxSelectionWarning');
    const saveBtn = document.getElementById('saveInvoiceBtn');
    if (!toggle || !warning || !saveBtn) return true;

    if (!toggle.checked) {
      warning.style.display = 'none';
      saveBtn.dataset.taxBlocked = '0';
      updateSaveButtonState();
      return true;
    }

    const anyChecked = document.querySelectorAll('.tax-check:checked, .invoice-tax-check:checked').length > 0;
    if (!anyChecked) {
      warning.style.display = 'block';
      saveBtn.dataset.taxBlocked = '1';
      updateSaveButtonState();
      return false;
    }
          ...
          visibleCols.forEach(col => {
            ...
            text = text.replace(/\s+/g, ' ').trim();
              ...
              if (baseColIndex !== null && col.index === baseColIndex) {
                ...
                let rowTaxSum = 0;
                let hasAppliedLineTax = false;
                const appliedTaxLabels = [];

                if (taxableOn) {
                  row.querySelectorAll('.tax-check').forEach((chk) => {
                    if (!chk.checked) return;
                    hasAppliedLineTax = true;
                    const rate = parseFloat(chk.dataset.taxRate || '0');
