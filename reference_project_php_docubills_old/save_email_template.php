...
if ($newNotifLabel !== '' && $newNotifSlug !== '') {
    ...
    try {
        ...
        // ✅ Force this save to use the new slug
        $notification_type = $newNotifSlug;

    } catch (Exception $e) {
        error_log("⚠️ Create notification failed: " . $e->getMessage());
...
// include design_json in your INSERT
$inserted = false;

try {
    // Try the newer/expected column name + timestamps
    $stmt = $pdo->prepare("
      INSERT INTO email_templates
        (template_name, html_content, design_json, assigned_notification_type, cc_emails, bcc_emails, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
    ");
    $stmt->execute([$template_name, $html_content, $design, $notification_type, $cc_emails, $bcc_emails]);
    $inserted = true;
    ...
    try {
        ...
        $stmt = $pdo->prepare("
          INSERT INTO email_templates
            (template_name, template_html, design_json, assigned_notification_type, cc_emails, bcc_emails, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
        ");
        $stmt->execute([$template_name, $html_content, $design, $notification_type, $cc_emails, $bcc_emails]);
        $inserted = true;
        ...
        try {
            ...
            $stmt = $pdo->prepare("
              INSERT INTO email_templates
                (template_name, html_content, design_json, assigned_notification_type, cc_emails, bcc_emails)
              VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$template_name, $html_content, $design, $notification_type, $cc_emails, $bcc_emails]);
            $inserted = true;
            ...
            $stmt = $pdo->prepare("
              INSERT INTO email_templates
                (template_name, template_html, design_json, assigned_notification_type, cc_emails, bcc_emails)
              VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$template_name, $html_content, $design, $notification_type, $cc_emails, $bcc_emails]);
            $inserted = true;
...
if (!$inserted) {
    $msg = urlencode('Failed to save template.');
    header("Location: manage-email-templates.php?error=$msg");
    exit;
}
...
// ✅ Redirect back to the saved template so dropdown auto-loads it
$success = urlencode('Email template saved successfully!');
header("Location: manage-email-templates.php?id={$newId}&success={$success}");
exit;
