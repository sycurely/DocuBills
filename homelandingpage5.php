                            ...
                            <div class="card">
                              <div class="invoice-box">
                                ...
                                <div class="inv-row-top">
                                  <div class="inv-logo">
                                    <div class="demo-logo-box">LOGO</div>
                                  </div>
...
(function () {
  ...
  function renderCompanyInfo(){
    ...
    if (companyInfoEl) {
      companyInfoEl.innerHTML = `
        <div class="company-name">${st.bill_to_name || 'Company Name'}</div>
        ${st.bill_to_rep ? `<div>${st.bill_to_rep}</div>` : ''}
        ${st.bill_to_address ? `<div>${st.bill_to_address}</div>` : ''}
        ${st.bill_to_phone ? `<div>${st.bill_to_phone}</div>` : ''}
        ${st.bill_to_email ? `<div>${st.bill_to_email}</div>` : ''}
      `;
    }

    // Render banking details in Bill To section if enabled
  ...
  function renderBankingDetails(){
    ...
    if (showBanking) {
      ...
      billToEl.innerHTML = `
        <div style="font-size: 12px; color: #6b7280; margin-bottom: 8px;"><strong>Payment Details:</strong></div>
        ${accountHolder ? `<div style="font-size: 12px;"><strong>Account Holder:</strong> ${accountHolder}</div>` : ''}
        ${bankName ? `<div style="font-size: 12px;"><strong>Bank:</strong> ${bankName}</div>` : ''}
        ${iban ? `<div style="font-size: 12px;"><strong>IBAN:</strong> ${iban}</div>` : ''}
        ${swift ? `<div style="font-size: 12px;"><strong>SWIFT/BIC:</strong> ${swift}</div>` : ''}
        ${accountNumber ? `<div style="font-size: 12px;"><strong>Account No:</strong> ${accountNumber}</div>` : ''}
        ${routingCode ? `<div style="font-size: 12px;"><strong>Routing Code:</strong> ${routingCode}</div>` : ''}
        ${paymentInstructions ? `<div style="font-size: 12px; margin-top: 8px; font-style: italic;">${paymentInstructions}</div>` : ''}
      `;
    } else {
      billToEl.innerHTML = '';
    }
